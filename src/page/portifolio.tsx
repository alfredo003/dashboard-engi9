...
import { getProducts, addProduct, updateProduct, deleteProduct } from '../../api/portifolio';
...
const Portifolio = () => {
  const [products, setProducts] = useState<Portifolio[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingProduct, setEditingProduct] = useState<Portifolio | null>(null);
  const [formData, setFormData] = useState<Omit<Portifolio, 'id' | 'image'>>({
    name: '',
    local: '',
    description: ''
  });
  ...
  const filteredProducts = products.filter(product =>
    ...
    (product.local || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleEdit = (product: Portifolio) => {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 mt-28">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Gerenciamento de Portfólio</h1>
          <p className="text-gray-600">Visualize, edite e gerencie o Portfólio</p>
        </div>
        ...
        <div className="mt-4 text-sm text-gray-600">
          Mostrando {filteredProducts.length} de {products.length} projetos
        </div>
      </div>

      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white max-w-4xl w-full my-8 rounded-xl">
            <div className="sticky top-0 bg-gradient-to-r from-gray-600 to-gray-700 px-8 py-6 flex justify-between items-center rounded-t-xl">
              <h2 className="text-2xl font-bold text-white">
                {editingProduct ? 'Editar Projeto' : 'Adicionar Projeto'}
              </h2>
              <button
                onClick={closeModal}
            ...
            <div className="p-8 space-y-8 max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-800">Imagem do Projeto</h3>
                </div>
              ...
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-800">Informações do Projeto</h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Nome do Projeto *</label>
                    <input
                      type="text"
                      value={formData.name}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                        errors.local ? 'border-red-400 bg-red-50' : 'border-gray-300'
                      }`}
            ...
            <div className="sticky bottom-0 bg-gray-50 px-8 py-4 rounded-b-xl flex justify-end space-x-3 border-t border-gray-200">
              ...
              >
                {uploading ? (
                  ...
                  <><Save className="h-5 w-5 mr-2" />{editingProduct ? 'Salvar Alterações' : 'Adicionar Projeto'}</>
                )}
              </button>
            </div>
          </div>
